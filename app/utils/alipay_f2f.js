...
module.exports = class alipay_f2f {
    ...
    constructor(config) {
        ...
        this._config["privateKey"] = config["privateKey"] || "";
    ...
    verifyCallback(postData) {
        ...
        return this._alipaySdk.checkNotifySign(postData);
    }
     ...
     * @return {Promise}
     */
    async createQRPay(option) {
        try {
            var tradeNo = option["tradeNo"] || "1572319737833";
            var subject = option["subject"] || "hello";
            // var body = option["body"] || "";
            // var biz_content = option["biz_content"] || "";
            var buyer_id = option["buyer_id"] || "111111";
            var totalAmount = option["totalAmount"] || "0.01";
            var timeExpress = option["timeExpress"] || 5;

            if (tradeNo == "") {
                return reject({
                    message: "tradeNo 参数不能为空.",
                    info: null
                });
            }
            ...
            var alipayArrayData = {};
            alipayArrayData["buyer_id"] = buyer_id;
            alipayArrayData["notify_url"] = this._config.notifyUrl;
            ...
            alipayArrayData["biz_content"] = JSON.stringify(alipayArrayData);

            const result = await this._alipaySdk.exec(
                'alipay.trade.precreate', alipayArrayData
            );
    ...
    //             return reject({
    //                 message: "alipayTradeNo 支付宝订单号不能为空.",
    //                 info: null
    //             });
    //         }
    //         var refundNo = option["refundNo"];
